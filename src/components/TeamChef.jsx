    ...
    <main className="py-10 px-4 md:px-10 rounded-md">
      <div className="flex items-center justify-between text-xl">
        ...
        <div className="flex items-center gap-x-3">
          <div
            onClick={handlePrev}
            className="w-8 flex items-center justify-center text-[#313235] text-2xl h-8 rounded-full border-2 border-[#f2bc57]"
          >
      ...
      <div
        ref={containerRef}
        className="image-container mt-4 overflow-x-scroll card"
      >
        <div className="whitespace-nowrap gap-y-4 flex md:gap-x-10">
          {popular.map((item) => (
            <div
              key={item.id}
              className="w-[320px] md:w-[600px] cards px-4 py-4 rounded-[14px] bg-white "
            >
              <div className="px-6 w-72 md:w-80 h-52 bg-slate-100 rounded-[14px]">
                <img
                  src={chefImg}
                  alt=""
                  className=" w-full h-full object-cover mt-3 rounded-md "
                />
              </div>
              <h1 className=" mx-auto py-4 text-2xl">
                {" "}
                Jane Ifiok, Victor Itodo, Ese Mary, Ada Ejim
              </h1>
            </div>
          ))}
        </div>
        ...
        <div className="whitespace-nowrap flex items-center gap-x-10 md:hidden">
          {popular.map((item) => (
            <div
              ...
              className={`${
                cI === item.id
                  ? "w-full px-12 py-4 rounded-md bg-white scroll-snap-align-center"
                  : "hidden"
              }`}
            >
              ...
              <div className="flex py-4 items-center justify-between">
                <h1>${item.price}</h1>
                <button className="px-2 py-2 bg-[#f2bc57] rounded-[10px]">
                  Add to Cart
                </button>
              </div>
            </div>
