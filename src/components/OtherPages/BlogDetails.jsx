      ...
      <div className=" md:px-10 relative">
        ...
        <div className="absolute top-0 left-0 w-full h-full flex flex-col text-white  justify-center z-10 items-center text-center">
          ...
          <p className="max-w-[314px] md:max-w-lg text-center mt-4">
            The chefs creativity knows no bounds. Every bite was a work of art,
            and the service was as impeccable as the cuisine.
          </p>
          <div className="flex px-4 mt-10 max-w-lg rounded-md border border-[#f2bc57] py-2">
            <input
              type="text"
              className="w-full outline-none bg-transparent placeholder:px-4"
        ...
        <div className="bg-white rounded-md py-10 px-3">
          <div className="flex flex-col md:flex-row  justify-between gap-x-10">
            <div className="w-full md:w-2/5">
              <img
                src={blogDetailArray[0]?.strMealThumb}
                alt=""
                className="rounded-[12px]"
              />
            </div>
            <div className="flex-1 flex flex-col ">
              <h1 className="text-3xl border-b pb-6 border-[#6C4B3F]  text-[#f2bc57]">
                {blogDetailArray[0]?.strMeal}
              </h1>
              <h1 className="mt-4 text-2xl text-[#6C4B3F] font-semibold">
                Category:
                <span className="font-light  text-md">
                  {" "}
                  {blogDetailArray[0]?.strCategory}
                </span>
              </h1>
              <h1 className="mt-4 text-2xl text-[#6C4B3F] font-semibold ">
                Source:
                <span className="font-light  text-md">
                  {" "}
                  {blogDetailArray[0]?.strSource}
                </span>
              </h1>
              <h1 className="mt-4 text-2xl text-[#6C4B3F] font-semibold  over">
                Tags:
                <span className="font-light  text-md">
                  {" "}
                  {blogDetailArray[0]?.strTags
                    ? blogDetailArray[0].strCategory
                    : "Nill"}
                </span>
              </h1>
          ...
          <div className="mt-4">
            <h1 className="text-[#6C4B3F] py-3 text-xl font-bold">
              Measurements:
            </h1>
          ...
          <div className="mt-4">
            ...
            <p>{blogDetailArray[0]?.strInstructions}</p>
          </div>
        </div>
      </div>
    </main>
